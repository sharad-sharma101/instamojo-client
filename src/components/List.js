...
const List = ({ ind, name, amount, status, id }) => {
  ...
  return (
    <div className="flex py-2 px-4 justify-between bg-white hover:bg-gray-200 ">
      <h3 className="text-lg flex-1 font-normal ">{ind}</h3>
      <h3 className="text-lg flex-1 font-normal w-1/4">{name}</h3>
      <h3 className="text-lg flex-1 font-normal w-1/4">Rs. {amount}</h3>
      ...
      <button
        ...
        className="text-blue-700 flex-1 border border-blue-700 hover:bg-blue-700 hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-full text-sm p-2.5 text-center inline-flex items-center dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:focus:ring-blue-800 dark:hover:bg-blue-500"
      >
        <AiFillDelete />
      </button>
      </div>
